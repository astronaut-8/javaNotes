...
![image-20220316174115768](https://typora---------image.oss-cn-beijing.aliyuncs.com/2bf43680-b8ed-474e-9336-6b3c74608ac2.png)
...
- 使用`Collections`装饰的线程安全集合，如： 
  ...
  - 说明：以上集合均采用**修饰模式设计**，**将非线程安全的集合包装后**，在调用方法时**包裹了一层synchronized代码块**。其并发性并不比遗留的安全集合好。
- java.util.concurrent.*

重点介绍`java.util.concurrent.* `下的线程安全集合类，可以发现它们有规律，里面包含三类关键词： **Blocking**、**CopyOnWrite**、**Concurrent**

- Blocking 大部分实现**基于锁**，并提供用来**阻塞**的方法 
- CopyOnWrite 之类容器修改开销相对较重 - **读多写少**
- Concurrent 类型的容器
  - 内部很多操作使用 **cas 优化**，一般可以提供**较高吞吐量** 
  - **弱一致性**
...
- ConcurrentHashMap**虽然每个方法都是线程安全的，但是多个方法的组合并不是线程安全的**
...
- 以上两部均是线程安全的。
- map 和 LongAdder由于是两个对象，所以交错执行不会有线程安全问题
...
<u>JDK 7 会把后加入的元素加入链表的头部，JDK8会把后加入的元素加到链表的尾部</u>

数组中元素超过**阈值**(四分之三)就会扩容(**hash重新取模，长度翻倍**)  

由于链表跟新 和 跟新数组索引引用(指向新链表) 的指令交错导致的
...
- JDK 8 虽然将扩容算法做了调整，不再将元素加入链表头（而是保持与扩容前一样的顺序），但仍**不意味着能 够在多线程环境下能够安全扩容**，还会出现其它问题（如扩容丢数据）
...
// 当初始化时, 为 -1
// 当扩容时, 为 -(1 + 扩容线程数)
// 当初始化或扩容完成后，为 下一次的扩容的阈值大小（max - 3/4）
...
// 在扩容前对象去get，发现为fnode，则会到新的对象去获取数据
...
// 当链表元素过多(8)，先会扩容(64之前)，后会将链表转化为红黑树，元素个数少了，又会将红黑树变回链表
...
public V put(K key, V value) {
    return putVal(key, value, false); // false - 每次都会用新值覆盖旧值
}
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    // 增加 size 计数
    addCount(1L, binCount); // 类似LongAdder 有多个累加单元 
    return null;
}
...
- 扩容，**扩容时以 bin 为单位进行**，需要对 **bin 进行 synchronized**，但这时妙的是其它竞争线程也不是无事可 做，它们会**帮助把其它 bin 进行扩容**，扩容时平均只有 1/6 的节点会把复制到新 table 中 
- size，元素个数保存在 baseCount 中，**并发时的个数变动保存在 CounterCell[] 当中**。最后统计数量时累加 即可(baseCount + cells)
...
![image-20220317180837785](https://typora---------image.oss-cn-beijing.aliyuncs.com/fc045cc2-31cc-4c5a-b132-7b1b7567549c.png)
...
![image-20220317180919562](https://typora---------image.oss-cn-beijing.aliyuncs.com/6c578a9a-016c-4cbe-b7bc-bc191ffd7751.png)

结果再与 this.segmentMask 做位于运算，最终得到 1010 即下标为 10 的 segment

![image-20220317180935914](https://typora---------image.oss-cn-beijing.aliyuncs.com/2eda5499-1fd8-4051-94fb-3c4e521dd28d.png)
...
- 在put的时候发生扩容 - rehash
- 扩容完成后才会去新增节点 跟新数组索引对应应用 并增加链表关系
...
![image-20220316215902936](https://typora---------image.oss-cn-beijing.aliyuncs.com/ce0e42b8-c367-4e28-b0e8-9fcffc1dc60d.png)

##### 当一个节点入队

`last = last.next = node;`

![image-20220316215925456](https://typora---------image.oss-cn-beijing.aliyuncs.com/b58e2cbb-a9f7-441c-8455-b4828f6df623.png)

再来一个节点入队`last = last.next = node;`

![image-20220316215949461](https://typora---------image.oss-cn-beijing.aliyuncs.com/29706d92-2dff-4161-b60d-8630a7c4668b.png)
...
![image-20220317142826808](https://typora---------image.oss-cn-beijing.aliyuncs.com/7a56c92f-8048-47db-b494-2338ffd7abc5.png)

`first = h.next`

![image-20220317142906729](https://typora---------image.oss-cn-beijing.aliyuncs.com/e8f73692-b5f4-4ea0-b4e3-3b15f31c05c8.png)

`h.next = h`

![image-20220317142924725](https://typora---------image.oss-cn-beijing.aliyuncs.com/5f645f0d-9e68-437e-916a-7a47a6c4419d.png)

`head = first`

![image-20220317142942832](https://typora---------image.oss-cn-beijing.aliyuncs.com/83a9c68f-da3e-465a-b30c-49b84efa4c4c.png)
...
![image-20220317143001497](https://typora---------image.oss-cn-beijing.aliyuncs.com/e6bdb925-f216-47cd-9894-dea8559fd314.png)



#### 加锁分析

**高明之处**在于用了**两把锁**和 dummy 节点 
...
**弱一致性**，不能保证后续的访问都能立即看到更新后的数据。在这种一致性模型下，系统不保证数据的实时一致性，而是在一定时间后，数据最终会达到一致状态，也称为 “最终一致性” 

##### get 弱一致性![image-20220317202641399](https://typora---------image.oss-cn-beijing.aliyuncs.com/ad838279-752d-4556-9ad3-d1d5ee3ca0d3.png)
