...
- **进程就可以视为程序的一个实例**。大部分程序可以**同时运行多个实例进程**（例如记事本、画图、浏览器 等），也有的程序只能启动一个实例进程（例如网易云音乐、360 安全卫士等）
...
- **进程基本上相互独立的**，而线程存在于进程内，是进程的一个**子集** 
...
- **线程更轻量**，线程**上下文切换成本**一般上要比进程上下文切换低

**上下文切换** - 在多任务处理过程中**切换 CPU 执行线程或进程的工作**。切换时会**保存**当前任务的执行状态（即“上下文”），然后**加载恢复**新任务的状态，切换到新任务后，任务可以继续**执行上次中断**的代码
...
- 多线程可以让方法执行变为异步
- 主线程中对于慢的操作，使用异步，新开一个线程，避免main主线程阻塞
...
1. 单核 cpu 下，**多线程不能实际提高程序运行效率**(由于要不断切换上下文，可能还有额外的时间开销)，只是为了能够在不同的任务之间切换，不同线程轮流使用 cpu ，不至于一个线程总占用 cpu，别的线程没法干活
2. 多核 cpu 可以并行跑多个线程，但**能否提高程序运行效率**还是要分情况的
   - 有些任务，经过精心设计，**将任务拆分，并行执行**，当然可以**提高程序的运行效率**。但不是所有计算任 务都能拆分（参考后文的【阿姆达尔定律】）
   - 也**不是所有任务都需要拆分**，任务的目的如果不同，谈拆分和效率没啥意义
3. IO 操作不占用 cpu，只是我们一般拷贝文件使用的是【阻塞 IO】，这时相当于**线程虽然不用 cpu，但需要一 直等待 IO 结束，没能充分利用线程**。所以才有后面的【非阻塞 IO】和【异步 IO】优化。